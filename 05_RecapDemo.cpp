...
const std::string g_WindowTitle{ "RecapDemo - Name, firstname - 1DAExx" };
...
const int g_NumBalls{ 20 };
Ball g_Balls[g_NumBalls]{};
const float g_VelocityScale{ 1.1f };
bool g_IsGravityEnabled{ false };
const float g_Gravity{ -9.81f * 100};
...
void InitGameResources( )
{
	for ( int idx{ 0 }; idx < g_NumBalls; ++idx )
	{
		// vel [50,150]
		// radius [5,50]
		// color [0.0f,1.0f]
		g_Balls[idx].velocity.x = rand( ) % ( 150 + 1 - 50 ) + 50.f;
		g_Balls[idx].velocity.y = rand( ) % 101 + 50.f;
		g_Balls[idx].radius = rand( ) % 46 + 5.0f;
		g_Balls[idx].color.r = rand( ) % 101 / 100.f;
		g_Balls[idx].color.g = rand( ) % 101 / 100.f;
		g_Balls[idx].color.b = rand( ) % 101 / 100.f;
		g_Balls[idx].color.a = 1.0f;
	}
}
void FreeGameResources( )
{
...
void ProcessKeyDownEvent( const SDL_KeyboardEvent  & e )
{
	...
	{
	case SDLK_r:
		g_Balls[0].color = Color4f{ 1.0f,0.0f,0.0f,1.0f };
		break;
	case SDLK_g:
		g_Balls[0].color = Color4f{ 0.0f,1.0f,0.0f,1.0f };
		break;
	case SDLK_b:
		g_Balls[0].color = Color4f{ 0.0f,0.0f,1.0f,1.0f };
		break;
	case SDLK_UP:
		g_Balls[0].velocity.x *= g_VelocityScale;
		g_Balls[0].velocity.y *= g_VelocityScale;
		break;
	case SDLK_DOWN:
		g_Balls[0].velocity.x /= g_VelocityScale;
		g_Balls[0].velocity.y /= g_VelocityScale;
		break;
	case SDLK_SPACE:
		g_IsGravityEnabled = !g_IsGravityEnabled;
		break;
	}
}
void ProcessKeyUpEvent( const SDL_KeyboardEvent  & e )
...
void Update( float elapsedSec )
{
	...
	if ( g_IsGravityEnabled )
	{
		for ( int idx{ 0 }; idx < g_NumBalls; ++idx )
		{
			g_Balls[idx].velocity.y += g_Gravity * elapsedSec;
		}
	}

	// Calculate the new position using the velocity
	for ( int idx{ 0 }; idx < g_NumBalls; ++idx )
	{
		g_Balls[idx].center.x += g_Balls[idx].velocity.x * elapsedSec;
		g_Balls[idx].center.y += g_Balls[idx].velocity.y * elapsedSec;

		// Right edge
		if ( g_Balls[idx].center.x + g_Balls[idx].radius > g_WindowWidth )
		{
			g_Balls[idx].velocity.x = -g_Balls[idx].velocity.x;
			g_Balls[idx].center.x = g_WindowWidth - g_Balls[idx].radius;
		}
		// Left edge
		if ( g_Balls[idx].center.x - g_Balls[idx].radius < 0 )
		{
			g_Balls[idx].velocity.x = -g_Balls[idx].velocity.x;
			g_Balls[idx].center.x = g_Balls[idx].radius;
		}
		// Bottom edge
		if ( g_Balls[idx].center.y - g_Balls[idx].radius < 0 )
		{
			g_Balls[idx].velocity.y = -g_Balls[idx].velocity.y;
			g_Balls[idx].center.y = g_Balls[idx].radius;
		}
		// Top edge
		if ( g_Balls[idx].center.y + g_Balls[idx].radius > g_WindowHeight )
		{
			g_Balls[idx].velocity.y = -g_Balls[idx].velocity.y;
			g_Balls[idx].center.y = g_WindowHeight - g_Balls[idx].radius;
		}
	}
}
...
void Draw( )
{
	...
	for ( int idx{ 0 }; idx < g_NumBalls; ++idx )
	{
		FillCircle( g_Balls[idx].center, g_Balls[idx].radius, g_Balls[idx].color );
	}
}
...
void FillCircle( const Point2f & center, float radius, const Color4f & color )
{
	...
	for ( float angle{ 0.0f }; angle < 2 * pi + deltaAngle; angle += deltaAngle )
	{
		...
		glVertex2f( center.x + radius * cos( angle ),
